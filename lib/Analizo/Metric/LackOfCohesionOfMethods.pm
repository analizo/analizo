...
Analizo::Metric::LackOfCohesionOfMethods - LackOFCohesionOfMethods4(LCOM4) metric
...
Article: Comparison of Various Lacks of Cohesion Metrics by Taranjeet Kaur 
and Rupinder Kaur.

See the paragraphs about LCOM4 in the article:

"LCOM3 varies in range between [0, 1]. LCOM3 indicates high cohesion and it is
also a well defined class, it show simplicity of class and high reusability of
class. A highly cohesive class provides high degree of encapsulation. LCOM3
formula is
...
that if we consider undirected graph G where vertices are methods of a class and
edge between vertices if corresponding methods at least share one instance variable."

"LCOM4 is like LCOM3 where graph G additionally has an edge between vertices
representing methods Mi and Mj if Mi invokes Mj. In other words also say that, it
measure number of components in a class. A connectd component is a set of related
methods."
