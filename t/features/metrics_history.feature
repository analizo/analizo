Feature: analizo metrics-history
  ...
  Scenario: non-code commits should not be analyzed
    When I explode t/samples/evolution.tar.gz
    And I run "analizo metrics-history --list ."
    Then the output must not match "ba62278e976944c0334103aa0044535169e1a51e"

  Scenario: merge commits without code change should not be analyzed
    When I explode t/samples/evolution.tar.gz
    And I run "analizo metrics-history --list ."
    Then the output must not match "0fdaaa7dcc8073332a957024fafc8c98f165e725"
  ...
  Scenario: actually processing initial commit and first commit after a non-relevant merge
    When I explode t/samples/evolution.tar.gz
    And I run "analizo metrics-history -o metrics.csv . && cat metrics.csv"
    Then the output must match "0d3c023120ad4e9f519a03fff275d048c52671ad,,"
