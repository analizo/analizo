...
sub feed {
  ...
  if ($line =~ m/^;; Function (\S+)\s*$/) {
    ...
    $self->{current_member} = $2;
  }

  # function calls/uses
  if ($line =~ m/^.*\(call.*"(.*)".*$/) {
    # direct calls
    $self->model->add_call($self->current_member, $1, 'direct');
  } elsif ($line =~ m/^.*\(symbol_ref.*"(.*)".*<function_decl\s.*$/) {
    # indirect calls (e.g. use of function pointers)
    $self->model->add_call($self->current_member, $1, 'indirect');
  }

  # variable references
  if ($line =~ m/^.*\(symbol_ref.*"(.*)".*<var_decl\s.*$/) {
    $self->model->add_variable_use($self->current_member, $1);
  }
